...
DATA_DIR=/tmp/arpari-data go run ./cmd/rebuild --supplier autronic-availability
...
curl http://localhost:8080/feeds/autronic-availability.xml
...
Endpoint `/feeds/autronic-availability.xml` vrací aktualizační skladový a cenový feed Autronicu. Čte sklad z `availability-feed.xml`, ale katalogový `product-feed.xml` používá jako filtr, zdroj ceny a variantního tvaru, aby update posílal jen produkty, které patří do našeho Autronic katalogu. Výstup obsahuje pouze identifikaci produktu nebo varianty, `PRICE_VAT`, povinný variantní parametr a `STOCK` se sklady po skladech; neposílá názvy, popisy, obrázky ani kategorie.

Endpoint `/feeds/sego.xml` vrací katalogový MVP ze SEGO Heureka feedu. Obsahuje kód, název, EAN, cenu s DPH, dostupnost, popis, obrázky a základní mapování do kancelářských židlí. Produkty uvedené v `internal/sego/excluded_products.csv` se podle stabilního EAN/CODE nepouští do výstupu, protože je klient spravuje ručně v Shoptetu. Endpoint `/feeds/sego-test.xml` vrací prvních 5 produktů. Produkční endpoint zůstává stejný i po přepnutí zdrojového SEGO feedu, aby nebylo nutné měnit automatický import v Shoptetu.

Endpoint `/feeds/hon.xml` vrací katalogový MVP z HON feedu. Obsahuje kód, název, výrobce/značku, dodavatele, cenu s DPH, sklad, dostupnost, popis, obrázky a doplňkové parametry; kategorie záměrně neposílá, aby pravidelný import nepřepisoval ruční zařazení produktů v Shoptetu. `SUPPLIER` zůstává `HON`; `MANUFACTURER` se u bezpečně rozpoznaných markerů ze zdrojové `MAIN_CATEGORY` mapuje na `Office Pro` nebo `LÖFFLER`, jinak zůstává `HON`. Položky se stejným `PRODUCT` a bezpečně rozpoznanou hodnotou z `DESCRIPTION` se skládají do variant s parametrem `Provedení`; nejasné skupiny zůstávají jako samostatné produkty. Endpoint `/feeds/hon-test.xml` vrací prvních 5 výstupních produktů.

Endpoint `/feeds/drevocal.xml` vrací katalogový MVP z Dřevočal B2B feedu. Jedna zdrojová položka je jedna varianta produktu; výstup skládá varianty podle `ITEMGROUP_ID` a používá `ITEM_ID` jako kód varianty. Matrace používají variantní parametry `Rozměr`, `Výška` a `Potah` a mapují se do `LOŽNICE > MATRACE`; lamelové rošty používají parametr `Rozměr` a mapují se do `LOŽNICE > ROŠTY`. Na varianty se přenáší dostupnost z `AVAILABILITY` a volitelný `GIFT` se posílá jako doplňkový parametr `Dárek`. Endpoint `/feeds/drevocal-test.xml` vrací prvních 5 výstupních produktů pro Shoptet test import.
