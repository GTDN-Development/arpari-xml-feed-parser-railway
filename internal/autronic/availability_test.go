...
func TestParseAvailabilityFiltersToCatalogAndMapsStock(t *testing.T) {
  ...
  <Product>
	...
	item := feed.Items[0]
	if item.Code != "NA-CHAIR-1" || item.Stock != "3" {
	...
	if item.Name != "" || item.PriceVAT != "" || len(item.Images) != 0 || item.DefaultCategory != nil {
		t.Fatalf("stock update must not carry catalog fields: %#v", item)
	}
}
...
func TestParseAvailabilityGroupsVariantStockWithParameters(t *testing.T) {
  ...
  <Product><ProductCode>CHAIR-WT</ProductCode><Availability><StockAvailabilityTotal Quantity="0" /></Availability></Product>
	...
	if variants[0].Code != "CHAIR-BK" || variants[0].Stock != "4" || variants[1].Code != "CHAIR-WT" || variants[1].Stock != "0" {
